...
var HttpClient = function(req, res) {
	...
	this._request = req;
	this._response = res;
	this._clientDisconnect = this.disconnect; // UGLY inheritance hack
	...
	if (req.headers.origin && !isAllowed(server.options.origins, req.headers.origin)) {
		server.options.logger('HttpClient :: Client denied due to disallowed origin (\'' + req.headers.origin + '\')', grappler.LOG.INFO);
	...
	if (self.state & grappler.STATE.PROTO_WEBSOCKET) {
		...
		req.connection.addListener('end', function() { self.emit('disconnect'); });
		...
		switch (req.method.toUpperCase()) {
			case "GET":
				...
				server.emit('connection', this);
				req.connection.addListener('end', function() { self.emit('disconnect'); });
				req.connection.addListener('close', function() { self.emit('disconnect'); });
			break;
			case "POST":
				var cookie;
...
HttpClient.prototype.disconnect = function() {
	...
	if (!(this.state & grappler.STATE.PROTO_WEBSOCKET) && !isMultipart && !isSSE) { // long poll
		// For when we disconnect without sending anything.
	...
	this._clientDisconnect.apply(this, arguments);
};
