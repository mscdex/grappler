...
var HttpClient = function(req, res) {
	...
	if (req.connection.client.state & grappler.STATE.PROTO_WEBSOCKET) {
		...
		if (draft == 75) {
			outgoingdata = outgoingdata.concat(['WebSocket-Origin: ' + req.headers.origin, 'WebSocket-Location: ws://' + req.headers.host]);
			...
			outgoingdata = outgoingdata.concat(['Sec-WebSocket-Origin: ' + req.headers.origin, 'Sec-WebSocket-Location: ws://' + req.headers.host + '/']);
		...
		req.connection.addListener('data', function(data) {
			var beginMarker = '\x00', endMarker = (draft == 75 ? '\ufffd' : '\xff'), curIdx, tmp;
			if (!inBuffer) {
		...
		switch (req.method.toUpperCase()) {
			case "GET":
				...
				res.setSecureCookie('grappler', req.connection.client.id);
...
HttpClient.prototype.write = function(data, encoding) {
	...
	if (this._request.connection.client.state & grappler.STATE.PROTO_WEBSOCKET) { // websocket
...
inherits(HttpClient, EventEmitter); // unnecessary?
