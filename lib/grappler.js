	...
	server.listeners('connection').unshift(function(socket) {
		...
		socket.addListener('data', function(buffer) {
			...
			if (flashSocketTest.indexOf("<policy-file-request/>") == 0) {
				...
				socket.end('<?xml version="1.0"?><!DOCTYPE cross-domain-policy SYSTEM "http://www.macromedia.com/xml/dtds/cross-domain-policy.dtd"><cross-domain-policy>' + allowedOrigins + '</cross-domain-policy>');
				socket.destroy();
			}
		});
