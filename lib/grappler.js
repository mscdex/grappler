...
// fnAcceptClient is a callback that determines if a given net.Stream is permitted to stay connected to the server,
// judging by the callback's return value. If a callback is not supplied, the default action is to accept all clients.
function Server(options/*, fnAcceptClient*/) {
	...
	this.options = mixin({
		...
		detectTimeout: 600 // time in ms to wait for an HTTP response before assuming a plain TCP client
	}, options || {});
	...
	this.close = function() {
		...
		for (var i=0,keys=Object.keys(connections),len=keys.length; i<len; ++i)
			connections[keys[i]].disconnect(false);
		self.emit('close');
	};
}
...
function Client(stream, srv) {
	var id = Math.floor(Math.random()*1e5).toString() + (new Date()).getTime().toString();
	var state = 0;
	var socket = stream;
	var server = srv;
	var self = this;
	...
	// Note: these methods should be part of the prototype, but sys.inherits doesn't work with an inheritance
	// chain (i.e.: HttpClient->Client->EventEmitter).
	...
	this.disconnect = function() {
		try {
			...
			// Setting the entry in the connections map to null/undefined performs better than using 'delete'.
			// However, this means you'll have an increasingly large map filled with empty spots.
			/*if (Object.keys(server.connections).indexOf(id) > -1) {
				
				this.server.connections[this.id] = undefined;
				// Hide the connection from "showing" in the connections hash
				Object.defineProperty(server.connections, id, { enumerable: false });
			}*/
			delete this.server.connections[id];
		} catch (e) {}
